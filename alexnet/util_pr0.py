...
def weight(shape, name, train):
	initial_pr0 = tf.truncated_normal(shape, stddev=0.01)
	w_pr0 = tf.Variable(initial_pr0, dtype=tf.float32, trainable=train, name=name)
	tf.add_to_collection('weights_pr0', w_pr0)
	return w_pr0

def bias(value, shape, name, train):
	initial_pr0 = tf.constant(value, dtype=tf.float32, shape=shape)
	return tf.Variable(initial_pr0, trainable=train, name=name)
