...
#    print(count)
    ...
    distance_list=[]
    for i in range(next_weights.shape[0]):
        distance_list.append(np.linalg.norm(next_weights[i]-next_weights_copy[i]))
    filename='clustdistancepcp'+str((n_clusters))+'.csv'
    np.savetxt(filename,distance_list,delimiter=',')
...
#        cluster_list[kmeans.labels_[i]].append(i)
        ...
        cluster_num_list=[]
        for i in range(len(cluster_list)):
            cluster_num_list.append(len(cluster_list[i]))

        filename='varpcp'+str(column_num)+'.txt'
        f = open(filename, 'w')
        f.write(str(np.var(cluster_num_list)))
        f.close()
#
