...
def alexnet(x):
	...
	with tf.name_scope('alexnetwork') as scope:
		with tf.name_scope('conv1') as inner_scope:
			wcnn1 = tu.weight([11, 11, 3, 96], name='wcnn1', std=0.01)
			bcnn1 = tu.bias(0.0, [96], name='bcnn1')
			conv1 = tf.add(tu.conv2d(x, wcnn1, stride=(4, 4), padding='VALID'), bcnn1)
...
#			pool1 = tu.max_pool2d(conv1, kernel=[1, 3, 3, 1], stride=[1, 2, 2, 1], padding='VALID')
		...
		with tf.name_scope('conv2') as inner_scope:
			wcnn2 = tu.weight([5, 5, 96, 256], name='wcnn2', std=0.01)
			bcnn2 = tu.bias(0.1, [256], name='bcnn2')
...
#			pool2 = tu.max_pool2d(conv2, kernel=[1, 3, 3, 1], stride=[1, 2, 2, 1], padding='VALID')
		...
		with tf.name_scope('conv3') as inner_scope:
			wcnn3 = tu.weight([3, 3, 256, 384], name='wcnn3', std=0.01)
		...
		with tf.name_scope('conv4') as inner_scope:
			wcnn4 = tu.weight([3, 3, 384, 384], name='wcnn4', std=0.01)
			bcnn4 = tu.bias(0.1, [384], name='bcnn4')
		...
		with tf.name_scope('conv5') as inner_scope:
			wcnn5 = tu.weight([3, 3, 384, 256], name='wcnn5', std=0.01)
			bcnn5 = tu.bias(0.1, [256], name='bcnn5')
...
def classifier(x, dropout):
	...
	with tf.name_scope('classifier') as scope:
		...
		with tf.name_scope('fullyconected2') as inner_scope:
			wfc2 = tu.weight([4096, 4096], name='wfc2', std=0.005)
			bfc2 = tu.bias(0.1, [4096], name='bfc2')
