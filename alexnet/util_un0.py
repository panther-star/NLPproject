...
def weight(shape, name, train):
	initial_un0 = tf.truncated_normal(shape, stddev=0.01)
	w_un0 = tf.Variable(initial_un0, dtype=tf.float32, trainable=train, name=name)
	tf.add_to_collection('weights_un0', w_un0)
	return w_un0

def bias(value, shape, name, train):
	initial_un0 = tf.constant(value, dtype=tf.float32, shape=shape)
	return tf.Variable(initial_un0, trainable=train, name=name)
...
def read_k_patches(image_path, k, fix):
	...
	img = cv2.imread(image_path)
	height, width, _ = img.shape
#	print("shape before resize: {}".format(img.shape))
#	if self.horizontal and np.random.random()<0.5:
   # 	img = cv2.flip(img,1)
	...
	if width <height:
		h = int(float(256 * height) /width)
		img = cv2.resize(img,(256, h),interpolation = cv2.INTER_AREA)
#		print("shape after resize: {}".format(img.shape))
	else:
		w = int(float(256 * width) / height)
		img = cv2.resize(img,(w, 256), interpolation = cv2.INTER_AREA)
#		print("shape after resize: {}".format(img.shape))
	...
	for i in range(k):
		# random 244x224 patch
		height, width, _ = img.shape
#		print("shape of image when patch:{}".format(img.shape))
		if fix:
			x = (width-224)/2
			y = (height-224)/2
		else:
...
#		print("x={},y={}".format(x,y))
		img_cropped=img[y:y+224, x:x+ 224]
#		print("shape of img_cropped:{}".format(img_cropped.shape))
		img_cropped-=IMAGENET_MEAN
		patches.append(img_cropped)
